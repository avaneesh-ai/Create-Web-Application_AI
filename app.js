...
const canManageUsers = () => {
  ...
  return localDeviceIds.includes(getDeviceId());
};
...
const upsertCurrentUser = () => {
  ...
  const savedRole = existing?.role || (users.length === 0 ? "admin" : "user");
...
const updateAccountDisplay = () => {
  ...
  adminAccessButton.textContent = canManageUsers() ? "Open admin access" : "Request admin access";
...
const createAssistantAnswer = (question) => {
  ...
  if (lowerQuestion.includes("admin") || lowerQuestion.includes("pre-admin")) {
    return "Admins and pre-admins can open the combined Admin / Pre-admin section only from a login on this computer. They can see user details, grant free Pro, give pre-admin access, suspend users, remove users, and send chat messages.";
  }
...
const handleIncomingLinks = () => {
  ...
  if (key === "pro" && value) {
    localStorage.setItem("pendingProEmail", decodeURIComponent(value));
  }
};
    ...
    row.innerHTML = `
      ...
      <td>
        <div class="admin-actions">
          ...
          <button class="mini-button" type="button" data-admin-action="pre-admin" data-user-id="${user.id}">Pre-admin</button>
          <button class="mini-button" type="button" data-admin-action="admin" data-user-id="${user.id}">Admin</button>
...
credentialsForm.addEventListener("submit", async (event) => {
  ...
  state.role = existingUser?.role || (getUsers().length === 0 ? "admin" : "user");
